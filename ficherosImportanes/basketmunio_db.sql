...
(
 ...
 comment 'ejemplo: equipo4-amarillo.png',
  `presupuesto_equipo` int not null default 180000000,
  `pts_equipo` int not null default 0
 comment 'puntos equipo',
  primary key (`id_equipo`,`id_usuario`,`id_liga`)
) engine = innodb
;
...
alter table `jugadores_equipos` add  primary key (`id_jugador`,`id_equipo`,`id_usuario`,`id_liga`)
;
...
(
 ...
 comment 'robos',
  `eff_jugador_partido_real` int(3) not null
 comment 'eficacia'
) engine = innodb
;
...
(
 ...
 comment 'robos',
  `eff_estadistica_total` float(4) not null
 comment 'eficacia'
) engine = innodb
;
...
alter table `jugadores_equipos` add constraint `relationship11` foreign key (`id_jugador`) references `jugadores` (`id_jugador`) on delete cascade on update cascade
;

alter table `jugadores_equipos` add constraint `relationship12` foreign key (`id_equipo`, `id_usuario`, `id_liga`) references `equipos` (`id_equipo`, `id_usuario`, `id_liga`) on delete cascade on update cascade
;

alter table `equipos` add constraint `relationship24` foreign key (`id_liga`) references `ligas` (`id_liga`) on delete cascade on update cascade
;
